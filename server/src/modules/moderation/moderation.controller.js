...
export const getReport = asyncHandler(async (req, res) => {
  const report = await moderationService.getReport(
    req.user,
    req.params.reportId,
  );
...
export const startReview = asyncHandler(async (req, res) => {
  const report = await moderationService.startReview(
    req.user,
    req.params.reportId,
  );
...
export const rejectReport = asyncHandler(async (req, res) => {
  const report = await moderationService.rejectReport(
    req.user,
    req.params.reportId,
    req.body.resolutionNote,
  );
...
export const updateUserStatus = asyncHandler(async (req, res) => {
  const user = await moderationService.updateUserStatus(
    req.user,
    req.params.userId,
    req.body.status,
    req.body.note,
