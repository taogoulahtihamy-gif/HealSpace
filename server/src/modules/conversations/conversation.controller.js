...
export const createDirectConversation = asyncHandler(
  async (req, res) => {
    const result = await createDirectConversationService(
      req.user.id,
      req.body,
    );
...
export const createGroupConversation = asyncHandler(
  async (req, res) => {
    const result = await createGroupConversationService(
      req.user.id,
      req.body,
    );
...
export const getConversations = asyncHandler(async (req, res) => {
  ...
  return ApiResponse.success(
    res,
    result,
    CONVERSATION_MESSAGES.LIST_FETCHED,
  );
});
